    ...
    var LakeFormMedia = {
        init: function() {
                ...
                .on('click', '.lake-form-media-submit', function(res){
                    ...
                    for (var i = 0; i < select_true_list.length; i++) {
                        urlList.push({
                            ['title'] : '',
                            ['content'] : '',
                            ['path']:$(select_true_list[i]).data('url')
                        });
                    }
                    ...
                    if (limit == 1) {
                        ...
                        var singleUrlList = [];
                        for(var i = 0; i < 1; i++) {
                            singleUrlList.push(urlList[i]);
                        }
                        urlList = singleUrlList;

                        single_urlList_json = JSON.stringify( urlList );

                        inputCont.val(single_urlList_json);
                        inputCont.attr("value", single_urlList_json);
                    } else {
                        // 提交限制数量
                        var newUrlList = [];
            ...
            $("body")
                .on('input', ".row.lake-form-media-img-show-row", function (key) {
                    ...
                    // 转为JSON结构，再赋值回去
                    var newJsonArray = JSON.stringify(originArray);
                    inputCont.val(newJsonArray);
                });
        },
        ...
        refreshPreview: function(name, urlList, options = []) {
            ...
            for (var i = 0; i < urlList.length; i++) {
                ...
                var content = urlList[i]['content'];
                var src = urlList[i]['path'];
                if (! this.isUrl(src)) {
